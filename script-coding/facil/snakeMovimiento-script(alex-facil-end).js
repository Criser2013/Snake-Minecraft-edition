...
Ejemplos: drawObstaclesm, drawObstaclesS (3,4), (5,4) -> image (3,4,20,20), image(5,4,20,20)
          drawObstaclesm, drawObstaclesS (10,19), (10,12) -> image (10,19,20,20), image(10,12,20,20)
*/
...
function colisionparedes () {
  ...
  else if (first(Mundo.snake).y<=-1&&Mundo.obstaculos.estatico.y==19) {
    if (Mundo.obstaculos.estatico.x==first(Mundo.snake).x) {
      Mundo.enemigos.primero.stop();
      Mundo.enemigos.segundo.stop();
      Mundo.enemigos.tercero.stop();
      Mundo.enemigos.cuarto.stop();
  ...
  else if (first(Mundo.snake).y>=20&&Mundo.obstaculos.estatico.y==0) {
    if (Mundo.obstaculos.estatico.x==first(Mundo.snake).x) {
      Mundo.enemigos.primero.stop();
      Mundo.enemigos.segundo.stop();
      Mundo.enemigos.tercero.stop();
...
function onTic(Mundo){
  ...
  //Esta condición determina si hubo colisión entre la cabeza del snake y la comida.
   ...
   //Esta condición es la encargada de hacer desaparecer el efecto de aumento de velocidad en la serpiente luego de 20 segundos 
    ...
    frameRate(fpscheat())
    return update(Mundo,{snake: moveSnake(Mundo.snake,Mundo.dir),trampas:verificadorTrampas(Mundo.snake,cheatposx(),cheatposy()),obstaculos:{movil:{x:Mundo.obstaculos.movil.x,y:Mundo.obstaculos.movil.y},estatico:{x:Mundo.obstaculos.estatico.x,y:Mundo.obstaculos.estatico.y},respawn:false},contador:0,reproductor:false})
  }
  ...
  else if ((first(Mundo.snake).x>=20||first(Mundo.snake).x<=-1)||((first(Mundo.snake).y<=-1)||(first(Mundo.snake).y>=20))) {
...
function onKeyEvent (Mundo, keyCode) {
  ...
  if (((keyCode==UP_ARROW||keyCode==87)&&Mundo.dir.y!==1)&&Mundo.colision==false){
    ...
    window.open("perdido(alex-facil-end).html","_self")
    return update(Mundo,{});
  }
  else {
    return update(Mundo,{});
...
document.body.style.backgroundImage = "url('images/fondo_end.png')";