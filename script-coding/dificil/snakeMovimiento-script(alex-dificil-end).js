...
Proposito: Determina la posición en "X" (posx) y "Y" (posy) de las trampas que spawnean contra el snake de forma aleatoria dentro del mapa.
Prototipo: drawCheat () {}
...
function colisionparedes () {
  ...
  //Estas 3 condiciones determinan si hubo colisión con una de las trampas estaticas que spawnean de forma aleatoria a partir de los 10 puntos.
    ...
    else if ((Mundo.contador==0&&Mundo.trampas.estado==false)&&Mundo.obstaculos.respawn==true) {
      return false
    }
    else {
      Mundo.enemigos.primero.stop();
...
function onKeyEvent (Mundo, keyCode) {
  ...
  if (((keyCode==UP_ARROW||keyCode==87)&&Mundo.dir.y!==1)&&Mundo.colision==false){
    ...
    window.open("perdido(alex-dificil-end).html","_self");
    return update(Mundo,{});
  }
  else {
    return update(Mundo,{});
...
document.body.style.backgroundImage = "url('images/fondo_end_dificil.png')";